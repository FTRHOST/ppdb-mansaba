...
export interface FormulirData {
  ...
  nisn?: string | null;
  nama?: string | null;
  tempatTanggalLahir?: string | null; // Combined field like "Batang, 19 Mei 2010"
  jenisKelamin?: 'Laki-laki' | 'Perempuan' | null;
  alamatLengkap?: string | null; // Combined full address
  desa?: string | null;
  kecamatan?: string | null;
  kabupaten?: string | null;
  provinsi?: string | null;
  dukuhJalan?: string | null; // Added based on form field
  rt?: string | null; // Added based on form field
  rw?: string | null; // Added based on form field
  noHp?: string | null;
  tinggal?: string | null; // Keterangan Tinggal
  ...
  noHpAyah?: string | null; // Added, use main noHp if separate not available
  namaIbu?: string | null;
  pendidikanIbu?: string | null;
  pekerjaanIbu?: string | null;
  noHpIbu?: string | null; // Added, use main noHp if separate not available
  ...
  alamatSekolahAsal?: string | null; // Just the city/area might be enough based on image
  rekomendasiPendaftaran?: string | null; // Who recommended
  punyaPiagam?: 'Punya' | 'Tidak Punya' | null; // Punya / Tidak
  motivasi?: string | null;
  tanggalDaftar?: Date | null; // Registration Date for signature
  tanggalCetak?: string | null; // Added for print date
}
     ...
     <div className="bg-white p-4 max-w-4xl mx-auto border border-gray-300 text-xs print:border-none print:shadow-none print:p-0">
       ...
       {/* Identitas Orang Tua / Wali */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 mb-2">
            ...
            <div className="border-r md:border-gray-300 md:pr-4">
                ...
                <DataRow label="No. HP" value={data.noHpAyah || data.noHp} /> {/* Fallback to student's HP */}
            ...
            <div className="mt-2 md:mt-0">
                 ...
                 <DataRow label="No. HP" value={data.noHpIbu || data.noHp} /> {/* Fallback to student's HP */}
            </div>
        </div>
         {/* Wali - Only show if Wali name exists */}
         {data.namaWali && (
       ...
       <SectionTitle title="PIAGAM / SERTIFIKAT" />
        <div className="mb-2">
           ...
           <DataRow label="Kejuaraan" value={data.punyaPiagam === 'Punya' ? '....................................' : '-'} /> {/* Placeholder */}
        </div>
