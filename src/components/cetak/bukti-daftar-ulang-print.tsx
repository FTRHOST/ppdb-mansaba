...
const DataRow: React.FC<{ label: string; value?: string | null; boldValue?: boolean }> = ({ label, value, boldValue }) => (
  value ? (
   ...
   <div className="flex mb-0.5 text-xs print:text-[8pt] print:leading-tight">
     <span className="w-24 flex-shrink-0 print:w-20">{label}</span> {/* Adjusted width */}
...
const ChecklistItem: React.FC<{ checked: boolean; label: string }> = ({ checked, label }) => (
    ...
    <div className="flex items-center mb-0.5 text-xs print:text-[8pt] print:leading-tight">
        {checked ? <CheckSquare className="w-2.5 h-2.5 mr-1 text-black print:w-2 print:h-2" /> : <Square className="w-2.5 h-2.5 mr-1 text-gray-400 print:w-2 print:h-2" />}
        <span>{label}</span>
    </div>
);
...
const Receipt: React.FC<{ data: BuktiDaftarUlangData; isArsip?: boolean }> = ({ data, isArsip = false }) => {
    ...
    return (
      // Added receipt-container class for flex item styling in print
      <div className={cn("receipt-container bg-white p-2 max-w-md mx-auto border border-black text-xs font-sans break-inside-avoid", isArsip ? "border-t-4 border-t-red-600" : "")}>
        {/* Header */}
        <div className="text-center mb-2 border-b-2 border-black pb-1">
           <div className="flex justify-center items-center mb-1">
                {/* Placeholder for logo */}
                <div className="w-10 h-10 bg-gray-200 flex items-center justify-center mr-2 rounded-full overflow-hidden flex-shrink-0 print:w-8 print:h-8 print:mr-1" data-ai-hint="school logo">
                     <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 text-green-700 print:w-5 print:h-5">
                ...
                <div className="flex-grow print:text-[9pt] print:leading-tight">
                    <h1 className="text-xs font-bold print:text-[8pt]">PANITIA</h1>
                    <h2 className="text-sm font-bold text-green-700 print:text-[10pt]">PENDAFTARAN PESERTA DIDIK BARU</h2>
                    <h2 className="text-base font-bold text-green-700 print:text-[10pt]">MA NU 01 BANYUPUTIH</h2>
                    <h3 className="text-xs font-bold print:text-[8pt]">TAHUN PELAJARAN 2025/2026</h3>
                </div>
           </div>
        </div>
         {isArsip && <p className="text-center text-xs font-semibold text-red-600 mb-1 print:text-[8pt]">Untuk Panitia</p>}
        <h3 className="font-bold text-center mb-2 underline text-sm print:text-[10pt] print:mb-1">BUKTI DAFTAR ULANG</h3>
        ...
        <div className="mb-2">
            ...
            <p className="mb-1 text-xs font-medium print:text-[8pt]">Berkas yang diserahkan:</p>
            ...
            <div className="mt-1 flex items-center">
                 <ChecklistItem checked={data.bayarDaftarUlang} label="Daftar Ulang" />
                  {data.bayarDaftarUlang && (
                     <span className="ml-2 bg-black text-white font-semibold px-1 py-0.5 rounded text-[9px] print:text-[7pt]"> {/* Adjusted size */}
        ...
        {/* Footer Box */}
        <div className={cn("border border-black p-1 text-center my-2 text-[9px] print:text-[7pt]", isArsip ? "min-h-[40px]" : "")}> {/* Adjusted size and padding */}
            {!isArsip && (
                <>
        ...
        <div className="flex justify-end mt-2 print:mt-1">
            <div className="text-center text-xs print:text-[8pt]">
                ...
                <p>Panitia PPDB</p>
                <br /> {/* Keep some space for signature */}
                <br />
...
export const BuktiDaftarUlangPrint: React.FC<{ data: BuktiDaftarUlangData }> = ({ data }) => {
   return (
      ...
      <div className="print-container flex flex-col md:flex-row gap-4 print:flex-row print:gap-4">
         ...
         <hr className="border-dashed border-gray-500 my-2 print:hidden" />
