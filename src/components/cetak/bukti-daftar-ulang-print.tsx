...
const ChecklistItem: React.FC<{ checked: boolean; label: string }> = ({ checked, label }) => (
    ...
    <div className="flex items-center mb-0_5 text-xs print:text-[8pt] print:leading-tight print:mb-0_5">
        {checked ? <CheckSquare className="w-2_5 h-2_5 mr-1 text-black print:w-2 print:h-2 print:mr-1" /> : <Square className="w-2_5 h-2_5 mr-1 text-gray-400 print:w-2 print:h-2 print:mr-1" />}
        <span>{label}</span>
    </div>
);
...
const Receipt: React.FC<{ data: BuktiDaftarUlangData; isArsip?: boolean }> = ({ data, isArsip = false }) => {
    ...
    return (
      ...
      <div className={cn("receipt-container bg-white p-1 max-w-none mx-auto border border-black text-xs font-sans break-inside-avoid print:p-1 print:text-xs", isArsip ? "border-t-4 border-t-red-600" : "")}>
        ...
        <div className="text-center mb-1 border-b-2 border-black pb-0.5 print:mb-1 print:pb-0.5">
           <div className="flex justify-center items-center mb-1 print:mb-0.5">
                ...
                <div className="flex-grow print:text-[8pt] print:leading-tight"> {/* Adjusted font size */}
                    <h1 className="text-xs font-bold print:text-[8pt]">PANITIA</h1>
                    <h2 className="text-sm font-bold text-green-700 print:text-[9pt]">PENDAFTARAN PESERTA DIDIK BARU</h2> {/* Adjusted font size */}
                    <h2 className="text-base font-bold text-green-700 print:text-[9pt]">MA NU 01 BANYUPUTIH</h2> {/* Adjusted font size */}
        ...
        <h3 className="font-bold text-center mb-1 underline text-sm print:text-[9pt] print:mb-1">BUKTI DAFTAR ULANG</h3> {/* Adjusted font size */}
        ...
        {/* Detail Daftar Ulang - Adjusted margin */}
        <div className="mb-1 print:mb-1">
            ...
            <div className="ml-1 grid grid-cols-1 print:ml-1"> {/* Single column and reduced margin */}
            ...
            <div className="mt-0.5 flex items-center print:mt-0.5">
                 <ChecklistItem checked={data.bayarDaftarUlang} label="Daftar Ulang" />
                  {data.bayarDaftarUlang && (
                     <span className="ml-1 bg-black text-white font-semibold px-0.5 py-0 rounded text-[8px] print:text-[7pt] print:ml-1 print:px-0.5 print:py-0"> {/* Adjusted size and padding */}
        ...
        {/* Footer Box - Adjusted size, padding, margin */}
        <div className={cn("border border-black p-0.5 text-center my-1 text-[8px] print:text-[7pt] print:my-1 print:p-0.5", isArsip ? "min-h-[30px]" : "")}>
            {!isArsip && (
                <>
        ...
        <div className="flex justify-end mt-1 print:mt-1">
            <div className="text-center text-xs print:text-[8pt]">
                ...
                <p>Panitia PPDB</p>
                <div className="h-5"></div> {/* Reduced space for signature */}
                <p className="font-bold underline print:font-bold">( Saniyah, S.H. )</p> {/* Hardcoded name */}
            </div>
        </div>
