...
const DataRow: React.FC<{ label: string; value?: string | null; boldValue?: boolean }> = ({ label, value, boldValue }) => (
  value || label === 'Alamat' ? ( // Always render Alamat row even if value is empty like image
   ...
   <div className="flex mb-0.5 text-xs print:text-[10pt] print:leading-tight print:mb-[0.5mm]"> {/* Match image tight spacing, small positive margin */}
     {/* Width for label adjusted to match image */}
     <span className="w-[110px] flex-shrink-0 print:w-[110px]">{label}</span> {/* Increased Label width */}
     <span className="mr-2 print:mr-[6px]">:</span> {/* Colon spacing */}
...
const ChecklistItem: React.FC<{ checked: boolean; label: string }> = ({ checked, label }) => (
    ...
    <div className="flex items-center mb-0 text-xs print:text-[10pt] print:leading-tight print:mb-0"> {/* No negative margin */}
        {checked ? <CheckSquare className="w-3 h-3 mr-1 text-black print:w-[10pt] print:h-[10pt] print:mr-[1.5mm]" /> : <Square className="w-3 h-3 mr-1 text-gray-500 print:w-[10pt] print:h-[10pt] print:mr-[1.5mm]" />} {/* Adjusted icon size & margin */}
        <span>{label}</span>
    </div>
);
...
const Receipt: React.FC<{ data: BuktiDaftarUlangData; isArsip?: boolean; namaPetugas?: string | null }> = ({ data, isArsip = false, namaPetugas }) => {
    ...
    return (
      ...
      <div className={cn("receipt-container bg-white p-2 max-w-full print:max-w-none print:p-[5mm_6mm] mx-auto border border-black text-xs font-sans break-inside-avoid print:text-[10pt] print:leading-normal flex flex-col h-full", isArsip ? "arsip" : "")}>
        {/* Header - Match image layout and styling */}
        <div className="text-center mb-1 border-b-2 border-black pb-1 print:mb-[1.5mm] print:pb-[1.5mm] print:border-b-[1.5pt]">
           <div className="flex justify-center items-center mb-0 print:mb-0"> {/* Reduced margin */}
                {/* Logo */}
                <div className="w-10 h-10 bg-transparent flex items-center justify-center mr-2 flex-shrink-0 print:w-[11mm] print:h-[11mm] print:mr-[2.5mm]" data-ai-hint="school logo green">
                     <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-full h-full text-green-700"> {/* Matched size */}
                ...
                <div className="flex-grow print:text-[9.5pt] print:leading-tight text-left"> {/* Left align text block */}
                    <p className="text-xs font-bold print:text-[9.5pt] mb-0">PANITIA APENDAFTARAN PESERTA DIDIK BARU</p> {/* Adjusted text */}
                    <p className="text-sm font-bold text-green-700 print:text-[10.5pt] mb-0">MA NU 01 BANYUPUTIH</p> {/* Larger school name */}
                    <p className="text-xs font-bold print:text-[9.5pt] mb-0">TAHUN PELAJARAN 2025 / 2026</p> {/* Added slash */}
                    {/* Divider */}
                    <div className="h-[2px] bg-teal-600 my-0.5 print:h-[1pt] print:my-[0.8mm]"></div> {/* Thinner line */}
                </div>
           </div>
        </div>
        {/* Title - Adjusted margin, added red line */}
        <h3 className="font-bold text-center mb-0.5 mt-0.5 underline text-sm print:text-[11pt] print:mb-[1mm] print:mt-[1mm]">
           BUKTI DAFTAR ULANG {isArsip ? '(ARSIP)' : ''}
        </h3>
        <div className="h-[2px] bg-red-600 w-1/3 mx-auto mb-2 print:h-[1pt] print:mb-[2.5mm]"></div> {/* Red line */}

         {/* Content Area - Ensure it grows */}
         <div className="receipt-content flex-grow">
            {/* Identitas Pendaftar - Adjusted margin */}
            <div className="mb-1 print:mb-[1.5mm]">
            ...
            <div className="mb-2 print:mb-[1.5mm]"> {/* Reduced bottom margin */}
                 <p className="mb-0.5 text-xs font-medium print:text-[10pt] print:mb-[1mm]">Kelengkapan:</p> {/* Label for checklist */}
                <div className="ml-2 grid grid-cols-1 gap-y-0 print:ml-[2mm] print:gap-y-[0.5mm]"> {/* Tighter grid */}
                ...
                <div className="mt-0.5 flex items-center print:mt-[1mm] ml-2 print:ml-[2mm]"> {/* Tighter spacing, aligned with checklist */}
                     <ChecklistItem checked={data.bayarDaftarUlang} label="Daftar Ulang" />
                      {data.bayarDaftarUlang && (
                         <span className="ml-2 bg-black text-white font-semibold px-1 py-0.5 rounded text-[9pt] print:text-[9.5pt] print:ml-[2mm] print:px-[1.5mm] print:py-[0.5mm]"> {/* Adjusted spacing/padding */}
